...
function heatLayersFunc(var1::Float64,var2::Float64)
    ...
    divNcell = 3# ceil(Int,sqrt(d_InAs_total*100))
    ...
    #if prot_quat_x != 1.0
    #stats = @timed (lVar, lPairs) = uOttawaSlabs_v2_1(Radiator_T, 300.0, divProtCell, divNcell, divPcell, divSubCell, d_firstgap, d_Rad, d_gap, d_prot, d_InAs_total, d_InAsSbP_base, d_InAs_sub, Si_ndoping, ndoping_InAs, pdoping_InAs, quat_x, prot_quat_x, mboxProt, mboxN, mboxP, mboxSub, xMinProt, xMinN, xMinP, xMinSub)
    #else
